...
// top-0 left-0 w-full , w-full to take the full length of the screen
...
const Header = () => {
  ...
  return (
    ...
    border-b border-n-6 lg:bg-n-8/90 lg:backdrop-blur-sm ${openNavigation ? 'bg-n-8' :
          ...
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map((item) => (
              ...
              className={`block relative font-code text-2xl uppercase text-n-1 transition-colors
                hover:text-color-1 ${item.onlyMobile ? 'lg:hidden' : ''} px-6 py-6 md:py-8
                lg:-mr-0.25 lg:text-xs lg:font-semibold ${item.url === pathname.hash ?
                  'z-2 lg:text-n-1' : 'lg:text-n-1/50'} lg:leading-5 lg:hover:text-n-1
                  xl:px-12`}
              >
                {item.title}
              </a>
